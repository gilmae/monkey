		...
		case string:
			if str, ok := evaluated.(*object.String); ok {
				if str.Value != expected {
					t.Errorf("string has wrong value, expected=%s, got=%s", expected, str.Value)
				}
			} else {
...
`,
	...
	for i, tt := range tests {
		...
		if errObj.Message != tt.expectedErrorMessage {
			t.Errorf("Test %d:Wrong error message, expected %q, got %q", i, tt.expectedErrorMessage, errObj.Message)
		}
	}
}
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("fn has wrong parameters, got %+v", fn.Parameters)
	}
	...
	if fn.Body.String() != expectedBody {
		t.Fatalf("fn has wrong body, expected %s, got %s", expectedBody, fn.Body.String())
	}
}
...
func TestAssignStatements(t *testing.T) {
	tests := []struct {
		...
		{"let a = 5; let b = a; let c = 5; c = a + b + c; c;", 15},
	}
